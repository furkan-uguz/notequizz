...
export function getQuestion(excludeSolfege1?: string | null, excludeSolfege2?: string | null, allowedOctaves?: Set<string>): Question {
	...
	if (allowedOctaves && allowedOctaves.size > 0) {
		const octavesArr = new Set(Array.from(allowedOctaves).map(Number));
		octaveFilteredNotes = notes.filter((n) => octavesArr.has(n.octave));
	}

	// Belirtilen solfej isimlerini (Do, Re vs.) içeren notaları havuzdan çıkarıyoruz.
	const availableNotes = octaveFilteredNotes.filter((n) => n.solfege !== excludeSolfege1 && n.solfege !== excludeSolfege2);
	...
	const wrongSolfegesInSameOctave = notes.filter((n) => n.octave === correctNote.octave && n.solfege !== correctNote.solfege).map((n) => n.solfege);
