...
import { playNoteSound } from "../lib/Sound";
...
export function NoteStaff({ note, volume = 0.5, noteSoundType = "piano", oscillatorType = "sine", interactive = true }: Readonly<NoteStaffProps>) {
	...
	return (
		<div className="relative w-full h-full max-w-md mx-auto flex items-center gap-1">
			<motion.div className="flex items-center w-25" animate={{ height: dynamicHeight }} transition={{ type: "spring", stiffness: 260, damping: 20 }}>
				{note.clef === "bass" ? <BassClef className="h-28 w-auto text-secondary" /> : <TrebleClef className="h-28 w-auto text-secondary" />}
			</motion.div>
			<div className="relative w-full">
				...
				{/* Note */}
				<motion.div
					className={cn("absolute right-1/2 sm:right-1/3 cursor-pointer transition-colors duration-300 text-secondary")}
					animate={{
				...
				>
					...
					/>
					<motion.div animate={{ scale: isActive ? 1.25 : 1 }}>
						<svg
							width="24"
							height="18"
							viewBox="0 0 38 24"
						...
						>
							<ellipse cx="18.6" cy="12" rx="18.6" ry="11.4" transform="rotate(-15 18.6 12)" fill="currentColor" />
						</svg>
					</motion.div>
				</motion.div>
			</div>
