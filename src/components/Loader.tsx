...
const Loader: FC<ILoader> = ({ ...props }: ILoader) => {
	...
	return (
		<Box
			className={
				"transition fade-in-out delay-150 duration-300 bg-black min-w-full " +
				(continueButtonClicked ? "opacity-0 " : "opacity-100 ") +
				(loadingBarCompleted ? "hover:cursor-pointer" : "opacity-100")
			}
			size={"sm"}
			mxAuto={true}
				...
				<FlexType flexType="flex-initial">
					...
					{loadingBarCompleted ? (
						<Flex align={"center"} justify={"center"}>
							<p className="text-white justify-center items-center text-2xl">Click To Continue</p>
						</Flex>
					) : (
						<Box className={"transition fade-in-out delay-150 duration-300 " + (loadingBarCompleted ? "opacity-0" : "opacity-100")}>
