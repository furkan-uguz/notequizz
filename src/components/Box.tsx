...
const Box: FC<IBoxInterface> = (props: IBoxInterface) => {
	const size = props.size === undefined ? "container" : containerSize[props.size];
	const mxAuto = props.mxAuto ? " mx-auto" : "";
	const classN = props.className === undefined ? "" : " " + props.className;
	const clazz = size + mxAuto + classN;
	return (
		<div className={clazz} onTransitionEnd={props.onTransitionEnd} onAnimationEnd={props.onAnimationEnd} onClick={props.onClick}>
			{props.children}
		</div>
	);
};
